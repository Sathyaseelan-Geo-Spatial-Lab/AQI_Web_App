...
os.environ["HTTP_PROXY"] = "http://ecsa_projectscientist1:sathya92125@jdmproxy.nrsc.gov.in:8080"
os.environ["HTTPS_PROXY"] = "http://ecsa_projectscientist1:sathya92125@jdmproxy.nrsc.gov.in:8080"

# Ensure a valid CA bundle is available for requests/Google auth.
# Some environments may set REQUESTS_CA_BUNDLE or SSL_CERT_FILE to an empty string or invalid path.
ca_bundle_path = certifi.where()
for env_key in ("REQUESTS_CA_BUNDLE", "SSL_CERT_FILE"):
    env_value = os.environ.get(env_key, "")
    if not env_value or not env_value.strip() or not os.path.exists(env_value.strip()):
        os.environ[env_key] = ca_bundle_path
    else:
...
# ---------------------------------------------------------------------------
# Google Earth Engine credentials (Render-safe)
# ---------------------------------------------------------------------------
# On Render, DO NOT ship the service-account key inside the repo. Instead set
# ONE of these environment variables in the Render dashboard:
#   * GEE_CREDENTIALS               -> the full service-account JSON as a string
#   * GOOGLE_APPLICATION_CREDENTIALS-> a path to the creds.json file (Secret File)
# For local development it falls back to <project>/config/creds.json
# ---------------------------------------------------------------------------
GEE_SCOPES = ["https://www.googleapis.com/auth/cloud-platform"]

_creds_json = os.environ.get("GEE_CREDENTIALS")
_creds_path = os.environ.get("GOOGLE_APPLICATION_CREDENTIALS") or os.path.join(
    base_dir, "config", "creds.json"
)

if _creds_json:
    credentials = service_account.Credentials.from_service_account_info(
        json.loads(_creds_json), scopes=GEE_SCOPES
    )
else:
    credentials = service_account.Credentials.from_service_account_file(
        _creds_path, scopes=GEE_SCOPES
    )
...
# ===========================================================================
# Column-averaged volume mixing-ratio conversion (dry-air column method)
# ---------------------------------------------------------------------------
...
#     SO2  -> ppm
#     HCHO -> ppm
# All are derived from the satellite total/tropospheric column (mol/m2) using
# the SAME dry-air column approach used for CO:
#
#     TC_dry_air = P_surf / (g * M_dry_air)  -  H2O_col * (M_H2O / M_dry_air)
#     mixing_ratio = pollutant_column / TC_dry_air           (mole fraction)
#     ppb = mixing_ratio * 1e9   ,   ppm = mixing_ratio * 1e6
#
# Surface pressure comes from ERA5-Land; the water-vapour column comes from the
# Sentinel-5P CO product (H2O_column_number_density).
# NOTE: NO2 is displayed in ppb (typical column-averaged values ~0.1-5 ppb),
# which reads far more naturally than ppm. SO2/HCHO column-averaged mixing ratios
# are physically very small, so in ppm they appear as tiny decimals (typically
# ~1e-4 to 1e-3 ppm); that is expected for column data.
# ===========================================================================
G_CONST = 9.80665          # Acceleration due to gravity (m/s^2)
M_DRY_AIR = 0.0289644      # Molar mass of dry air (kg/mol)
M_H2O = 0.01801528         # Molar mass of water vapour (kg/mol)

# pollutant -> (multiplier, display unit). 1e9 => ppb, 1e6 => ppm
DRY_AIR_MIXING_RATIO = {
    'CO':   {'multiplier': 1e9, 'unit': 'ppb', 'decimals': 3},
    'NO2':  {'multiplier': 1e9, 'unit': 'ppb', 'decimals': 2},
    'SO2':  {'multiplier': 1e6, 'unit': 'ppm', 'decimals': 2},
    'HCHO': {'multiplier': 1e6, 'unit': 'ppm', 'decimals': 2},
}

# pollutant -> (image collection id, column band name in mol/m2)
MIXING_RATIO_SOURCE_BAND = {
...
def compute_tc_dry_air_image(bounds, start_date, end_date):
    """Total column of dry air (mol/m^2) as a mean ee.Image over the period.
    ...
    h2o = (
        ...
        .map(lambda im: im.updateMask(im.gte(0)))
        .mean()
    )
    return surface_pressure.divide(G_CONST * M_DRY_AIR).subtract(
        h2o.multiply(M_H2O / M_DRY_AIR)
    )


def column_to_mixing_ratio(column_img, tc_dry_air, pollutant):
    """Convert a pollutant column image (mol/m^2) into a column-averaged
    volume mixing ratio scaled to ppb/ppm per DRY_AIR_MIXING_RATIO."""
    multiplier = DRY_AIR_MIXING_RATIO[pollutant]['multiplier']
    return column_img.divide(tc_dry_air).multiply(multiplier)


def mixing_ratio_timeseries(pollutant, bounds, geometry, start_date, end_date, scale=1000):
    """Daily column-averaged mixing-ratio (ppb/ppm) time series for CO/NO2/SO2/HCHO
    using the dry-air column method. Returns a list of {date, value, band}."""
    coll_id, band = MIXING_RATIO_SOURCE_BAND[pollutant]
    multiplier = DRY_AIR_MIXING_RATIO[pollutant]['multiplier']
    ...
    def compute_daily(date):
        date = ee.Date(date)
        gas_img = gas_coll.filterDate(date, date.advance(1, 'day')).mean()
        h2o_img = h2o_coll.filterDate(date, date.advance(1, 'day')).mean()
        pressure_img = pressure_coll.filterDate(date, date.advance(1, 'day')).mean()

        valid = (
            gas_img.bandNames().size().gt(0)
            .And(h2o_img.bandNames().size().gt(0))
            .And(pressure_img.bandNames().size().gt(0))
        )

        def feature():
            tc_dry = pressure_img.divide(G_CONST * M_DRY_AIR).subtract(
                h2o_img.multiply(M_H2O / M_DRY_AIR)
            )
            mr = gas_img.divide(tc_dry).multiply(multiplier).rename('mr')
            mean_dict = mr.clip(geometry).reduceRegion(
            ...
            return ee.Feature(None, {
                ...
                'value': mean_dict.get('mr')
            })

        return ee.Algorithms.If(valid, feature(), None)

    dates = gas_coll.aggregate_array('system:time_start')
    date_list = ee.List(dates).map(lambda d: ee.Date(d))
    feature_collection = (
        ee.FeatureCollection(date_list.map(compute_daily))
    ...
    for feature in features_list:
        ...
        v = props.get('value')
        if v is not None:
            results.append({'date': d, 'value': v, 'band': pollutant})
    return results
...
def get_pollutant():
    try:
        ...
        if pollutant == 'PM10':
            ...
            # Convert NO2 column (mol/m2) -> column-averaged mixing ratio (ppb)
            tc_dry_air = compute_tc_dry_air_image(buffered_city_geometry, start_date, end_date)
            pollutant_mean = column_to_mixing_ratio(NO2_mean, tc_dry_air, 'NO2').rename('NO2')
            ...
            _dec = DRY_AIR_MIXING_RATIO['NO2']['decimals']
            min_value = round(min_value, _dec)
            max_value = round(max_value, _dec)

            if min_value == 0 and max_value == 0:
                return jsonify({'error': 'NO2 data is too low or not available for visualization in this area/date range.'}), 404

            unit = DRY_AIR_MIXING_RATIO['NO2']['unit']
...
#### Final Corrected Output as per CPCB was Completed No Need To Change ########### 
            ...
            if filtered_collection.size().getInfo() == 0 or surface_pressure_collection.size().getInfo() == 0:
                ...
                if mean_image is None:
                    ...
                    # CO total column (mol/m2); converted to ppb after the if/else
                    CO_mean = mean_image
                    buffered_city_geometry = new_geom
                ...
                # Mean CO total column (mol/m2); converted to ppb after the if/else
                CO_mean = filtered_collection.select(
                    'CO_column_number_density'
                ).mean().clip(buffered_city_geometry)

            # ------------------------------------------------------------------
            # Convert CO column (mol/m2) -> column-averaged mixing ratio (ppb)
            # using the dry-air column method.
            # ------------------------------------------------------------------
            tc_dry_air = compute_tc_dry_air_image(buffered_city_geometry, start_date, end_date)
            pollutant_mean = column_to_mixing_ratio(CO_mean, tc_dry_air, 'CO').rename('CO')
            ...
            print("CO percentiles:- ", percentiles)

            min_value = percentiles.get('CO_p5', None)
            max_value = percentiles.get('CO_p95', None)

            if min_value is None or max_value is None:
                return jsonify({'error': 'Could not calculate data range for CO.'}), 500

            _dec = DRY_AIR_MIXING_RATIO['CO']['decimals']
            min_value = round(min_value, _dec)
            max_value = round(max_value, _dec)

            if min_value == 0 and max_value == 0:
                return jsonify({'error': 'CO data is too low or not available for visualization in this area/date range.'}), 404

            unit = DRY_AIR_MIXING_RATIO['CO']['unit']
            ...
            # Convert SO2 column (mol/m2) -> column-averaged mixing ratio (ppm)
            tc_dry_air = compute_tc_dry_air_image(buffered_city_geometry, start_date, end_date)
            pollutant_mean = column_to_mixing_ratio(SO2_mean, tc_dry_air, 'SO2').rename('SO2')
            ...
            _dec = DRY_AIR_MIXING_RATIO['SO2']['decimals']
            min_value = round(min_value, _dec)
            max_value = round(max_value, _dec)
            ...
            unit = DRY_AIR_MIXING_RATIO['SO2']['unit']
            ...
            # Convert HCHO column (mol/m2) -> column-averaged mixing ratio (ppm)
            tc_dry_air = compute_tc_dry_air_image(buffered_city_geometry, start_date, end_date)
            pollutant_mean = column_to_mixing_ratio(HCHO_mean, tc_dry_air, 'HCHO').rename('HCHO')
            ...
            _dec = DRY_AIR_MIXING_RATIO['HCHO']['decimals']
            min_value = round(min_value, _dec)
            max_value = round(max_value, _dec)
            ...
            unit = DRY_AIR_MIXING_RATIO['HCHO']['unit']
...
POLLUTANT_CONFIGS = {
    ...
    'NO2': {
        ...
        'unit': 'ppb',   # column-averaged mixing ratio via dry-air method (x1e9)
        'scale_factor':50,
        'offset':0,
    },
    'CO': {
        ...
        'unit': 'ppb',   # column-averaged mixing ratio via dry-air method
    ...
    'SO2': {
        ...
        'unit': 'ppm',   # column-averaged mixing ratio via dry-air method
...
def process_pollutant_data(geometry_data, pollutant, start_date, end_date, scale=1000, allow_interpolation=False):
    ...
    if collection.size().getInfo() == 0 and allow_interpolation:
        ...
        # Apply scale factor/offset only for pollutants NOT converted via the
        # dry-air mixing-ratio method (CO/NO2/SO2/HCHO are handled below).
        if pollutant not in DRY_AIR_MIXING_RATIO and 'scale_factor' in config:
            mean_image = mean_image.multiply(config['scale_factor'])
            if 'offset' in config:
                mean_image = mean_image.add(config['offset'])

    # Convert gas columns (mol/m2) -> column-averaged mixing ratios (ppb/ppm)
    if pollutant in DRY_AIR_MIXING_RATIO:
        tc_dry_air = compute_tc_dry_air_image(bounds, start_date, end_date)
        mean_image = column_to_mixing_ratio(mean_image, tc_dry_air, pollutant)
...
def compute_timeseries_mean(geometry_data, pollutant, start_date, end_date, scale=1000):
    ...
    # Gases converted via the dry-air column method: CO -> ppb, NO2 -> ppb, SO2/HCHO -> ppm
    if pollutant in DRY_AIR_MIXING_RATIO:
        return mixing_ratio_timeseries(
            pollutant, bounds, geometry, start_date, end_date, scale
        )
...
def compute_timeseries_mean_3(geometry_data, pollutant, start_date, end_date, scale=1000):
    ...
    # Gases converted via the dry-air column method: CO -> ppb, NO2 -> ppb, SO2/HCHO -> ppm.
    # Uses the same TC_dry_air approach as the map legend so graph units match.
    if pollutant in DRY_AIR_MIXING_RATIO:
        return mixing_ratio_timeseries(
            pollutant, bounds, geometry, start_date, end_date, scale
        )
...
def get_time_series():
    try:
        ...
        # Gases converted via the dry-air column method: CO -> ppb, NO2 -> ppb, SO2/HCHO -> ppm
        if pollutant in DRY_AIR_MIXING_RATIO:
            _dec = DRY_AIR_MIXING_RATIO[pollutant]['decimals']
            ts = mixing_ratio_timeseries(
                pollutant, point.bounds(), point, start_date, end_date, scale=1000
            )
            series_data = [
                {'date': r['date'], 'value': round(float(r['value']), _dec)}
                for r in ts if r.get('value') is not None
            ]
            ...
            for dp in series_data:
                if dp['date'] not in seen_dates:
                    ...
                    unique_series_data.append(dp)
            if not unique_series_data:
                return jsonify({'error': 'No data available for the specified dates.'}), 404
            return jsonify({
                'series': unique_series_data,
                'unit': DRY_AIR_MIXING_RATIO[pollutant]['unit'],
                'dates': [dp['date'] for dp in unique_series_data]
            })
...
if __name__ == '__main__':
    # On Render, gunicorn (see Procfile) serves the app and binds to $PORT.
